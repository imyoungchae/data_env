...
for i in range(num_envs):
    ...
    ur5_actor = gym.create_actor(env, ur5_asset, gymapi.Transform(r=gymapi.Quat.from_axis_angle(gymapi.Vec3(0.0, 0.0, 1.0), -np.pi/2),p=gymapi.Vec3(1.5,2.0,0.9)),'ur5',i,0)
    ...
    sink_actor = gym.create_actor(env, sink_asset, gymapi.Transform(r=gymapi.Quat.from_axis_angle(gymapi.Vec3(0.0, 0.0, 1.0), np.pi/2),p=gymapi.Vec3(2.7-1.125,1.93-1.21,0)),'sink',i,0)
    fridge_actor = gym.create_actor(env, fridge_asset, gymapi.Transform(r=gymapi.Quat.from_axis_angle(gymapi.Vec3(0.0, 0.0, 1.0), np.pi/2),p=gymapi.Vec3(0.35,2.655,0.0)),'fridge',i,0)
    desk_actor = gym.create_actor(env, tomato_asset, gymapi.Transform(r=gymapi.Quat.from_axis_angle(gymapi.Vec3(0.0, 0.0, 1.0), np.pi),p=gymapi.Vec3(1.35,1.8,0.8)),'desk',i,0)
    ...
    cooker_actor = gym.create_actor(env, frypan_asset, gymapi.Transform(p=gymapi.Vec3(2.7-0.935,1.93-1.15,0.75),r=gymapi.Quat.from_axis_angle(gymapi.Vec3(0.0, 0.0, 1.0), -np.pi/2)),'cooker',i,0)
    object_position.y = object_position.y - 0.3
    object_position.x = object_position.x + 0.05
    toast_actor = gym.create_actor(env, knife_asset, gymapi.Transform(p=gymapi.Vec3(2.7-1.425,1.93-1.31,0.8),r=gymapi.Quat.from_axis_angle(gymapi.Vec3(0.0, 0.0, 1.0), -np.pi/2)), 'toast', i, 0)
    object_position.x = object_position.x - 0.5
    can_actor = gym.create_actor(env, mustard_asset, gymapi.Transform(p=gymapi.Vec3(2.7-1.125,1.93-1.11,0.83),r=gymapi.Quat.from_axis_angle(gymapi.Vec3(0.0, 0.0, 1.0), -np.pi)), 'can', i, 0)
    #camera
...
joint_positions = np.array([-0.4745,-1.4679, 1.9761, -2.0973,-1.6185,-1.5006,0.5000, 0.5000,-0.5000,-0.5000], dtype=np.float32)
