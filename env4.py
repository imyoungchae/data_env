...
for i in range(8):
    ur5_dof_props['stiffness'][i] = 800
    ur5_dof_props['damping'][i] = 200
    ur5_dof_props['driveMode'][i] = gymapi.DOF_MODE_POS

asset_box = gym.create_box(sim,0.5,0.5,0.5,wall_options)
...
for i in range(num_envs):
    ...
    ur5_actor = gym.create_actor(env, ur5_asset, gymapi.Transform(r=gymapi.Quat.from_axis_angle(gymapi.Vec3(0.0, 0.0, 1.0), -np.pi/2),p=gymapi.Vec3(1.5,2.0,0.5)),'ur5',i,0)
    gym.set_actor_dof_properties(env, ur5_actor, ur5_dof_props)
    gym.create_actor(env, asset_box, gymapi.Transform(p=gymapi.Vec3(1.5,2.0,0.25)),'box',i,0)
    sink_actor = gym.create_actor(env, sink_asset, gymapi.Transform(p=gymapi.Vec3(0.37,1.8,0.0)),'sink',i,0)
    fridge_actor = gym.create_actor(env, fridge_asset, gymapi.Transform(r=gymapi.Quat.from_axis_angle(gymapi.Vec3(0.0, 0.0, 1.0), -np.pi/2),p=gymapi.Vec3(2.675,2.655,0.0)),'fridge',i,0)
    desk_actor = gym.create_actor(env, desk_asset, gymapi.Transform(r=gymapi.Quat.from_axis_angle(gymapi.Vec3(0.0, 0.0, 1.0), np.pi),p=gymapi.Vec3(2.7-0.125,1.93-0.21,0.0)),'desk',i,0)
    ...
    cooker_actor = gym.create_actor(env, cooker_asset, gymapi.Transform(p=object_position,r=gymapi.Quat.from_axis_angle(gymapi.Vec3(0.0, 0.0, 1.0), -np.pi/2)),'cooker',i,0)
    object_position.y = object_position.y - 0.3
    object_position.x = object_position.x + 0.05
    toast_actor = gym.create_actor(env, toast_asset, gymapi.Transform(p=object_position,r=gymapi.Quat.from_axis_angle(gymapi.Vec3(0.0, 0.0, 1.0), -np.pi/2)), 'toast', i, 0)
    object_position.x = object_position.x - 0.5
    #can_actor = gym.create_actor(env, can_asset, gymapi.Transform(p=object_position,r=gymapi.Quat.from_axis_angle(gymapi.Vec3(0.0, 0.0, 1.0), -np.pi/2)), 'can', i, 0)
    #camera
        ...
        if evt.action == "screenshot" and evt.value > 0:
            for i in range(num_envs):
                for j in range(3):
                    ...
                    print("writing environment/rgb_image_%03d_%03d.png" % (i,  j))
                    rgb_image_filename = "environment/rgb_image_%03d_%03d.png" % (i, j)
                    gym.write_camera_image_to_file(sim, envs[i], cam_handles[i][j], gymapi.IMAGE_COLOR, rgb_image_filename)
